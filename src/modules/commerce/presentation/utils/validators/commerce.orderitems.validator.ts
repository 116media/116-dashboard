import type { Rule } from "antd/es/form";
import { ValidatorUtils } from "@/shared/presentation/utils/validators/validators.utils";

/**
 * Validation rules for order item forms.
 *
 * @description
 * Provides reusable validation rules for Ant Design Form components.
 * Matches the backend's FluentValidation constraints.
 *
 * @remarks
 * All methods return Rule arrays compatible with Ant Design Form's rules prop.
 */
export const OrderItemsValidator = {
    /**
     * Validates content kind selection field.
     *
     * @param {string} label - Display name for error messages
     * @returns {Rule[]} Array of validation rules
     *
     * @remarks
     * - Required field
     */
    contentKind: (label: string): Rule[] => [ValidatorUtils.required(label)],

    /**
     * Validates category selection field.
     *
     * @param {string} label - Display name for error messages
     * @returns {Rule[]} Array of validation rules
     *
     * @remarks
     * - Required field
     */
    categoryId: (label: string): Rule[] => [ValidatorUtils.required(label)],

    /**
     * Validates promotion level selection field.
     *
     * @param {string} label - Display name for error messages
     * @returns {Rule[]} Array of validation rules
     *
     * @remarks
     * - Required field
     */
    promotionLevelId: (label: string): Rule[] => [ValidatorUtils.required(label)],

    /**
     * Validates pricing tier selection field.
     *
     * @param {string} label - Display name for error messages
     * @returns {Rule[]} Array of validation rules
     *
     * @remarks
     * - Required field
     */
    pricingTierId: (label: string): Rule[] => [ValidatorUtils.required(label)]
} as const;
